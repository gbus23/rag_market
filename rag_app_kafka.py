...
Consomme les articles du topic Kafka et les analyse avec un LLM via Ollama.
...
Provide a concise analysis (2-3 sentences) based on the articles above. Be factual and reference the sources."""
        ...
        # 5. Query LLM via Ollama
        try:
            response = self._call_ollama(prompt)
            return response
        except Exception as e:
            # 🔎 DEBUG : afficher le vrai type d'erreur et son message
            print("### DEBUG EXCEPTION TYPE:", type(e), "MESSAGE:", repr(e))
            return (
                f"Error calling LLM via Ollama: {e}\n\n"
                f"Make sure Ollama is running at {OLLAMA_BASE_URL} "
                f"and the model '{MODEL_NAME}' is available.\n"
                f"Example:\n"
                f"  ollama run {MODEL_NAME} \"Hello\"\n"
            )
    ...
    def _call_ollama(self, prompt: str) -> str:
        ...
        try:
            # timeout en secondes (connect_timeout, read_timeout)
            ...
            raise Exception(
                ...
                "Try reducing the number of articles or check system load."
            )
        except Exception as e:
